...
@section('content')
<link href="{{ asset('css/teachers/create.css') }}" rel="stylesheet">
<div class="container">
...
@if(auth()->check() && auth()->user()->role === 'Admin'  && auth()->user()->status == '1')
                <div class="card-header">إضافة معلم جديد</div> <!-- Add New Teacher -->
...
<form method="POST" action="{{ route('teachers.store') }}">
  ...
  <div class="form-group row">
                      ...
                      <div class="form-group row">
                            ...
                            for="phone" class="col-md-4 col-form-label text-md-right">رقم الهاتف</label>
                            <div class="col-md-6">
                                <input pattern="\d{11}"
                          title="يجب ان يحتوي على 11 رقم"
                                id="phone" type="tel" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ old('phone') }}" required autocomplete="phone">
                                @error('phone')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
    ...
    <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select id="user_id" class="form-control @error('user_id') is-invalid @enderror" name="user_id" required autocomplete="user_id" autofocus>
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('user_id') == $user->id ? 'selected' : '' }}>{{ $user->user_name }}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                    <span class="invalid-feedback" role="alert">
              ...
              @elseif(auth()->check() && auth()->user()->role === 'Teacher'  && auth()->user()->status == '1')
                              ...
                              <div class="card-header">إكمال معلوماتك كمعلم جديد</div> <!-- Add New Teacher -->
...
@endsection
@push('scripts')
<script src="{{ asset('js/teachers/create.js') }}"></script>
@endpush